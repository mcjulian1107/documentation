...
Qweb collectors result are a little different from other collectors in term of collected data, and can be analysed from the ir_profile view form using a custom widget
.. todo image
...
.. code-block:: python
    ...
    with db.cursor() as cr:
        with odoo.tools.profiler.Profiler(db=db.dbname):
           ...
...
Good to know
------------
- Be aware of randomness, multiple execution can lead to different results.
...
- Profiling result can be memory intensive. in some cases (profiling an install, or a long request) it is possible that you may reach memory limit, especially when rendering the speedsope result leading to a 500 error and a message saying "Unrecognised format". In this case you may need to start the server with an higher memory limit locally. `--limit-memory-hard $((8*1024**3))`
